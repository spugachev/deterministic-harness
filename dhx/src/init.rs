...
/// Fetch one embedded scaffold file's text by its asset-relative path (used by
/// the Dockerfile renderer, C12).
pub(crate) fn scaffold_file(rel: &str) -> Option<String> {
    SCAFFOLD
        .get_file(rel)
...
pub(crate) fn run(path: &str, name: Option<&str>, force: bool) -> Result<()> {
    ...
    // Generate the lifecycle TLA from the scaffolded FSM source (C5), and the
    // README counts region + traceability lock + Dockerfile, so the fresh
    // project is green immediately.
    ...
    crate::docker::render_dockerfile(&cfg, false)?;

    println!(
        "\n✓ dhx init done. Next:\n  cd {}\n  dhx check              # cheap gates\n  dhx verify --quick     # full quick tier\n  dhx verify --full      # heavy tier (Docker)",
        target.display()
    );
    Ok(())
}
