...
/// transition function. `verus!{}`-wrapped sources work too because syn parses
/// the macro body's items once the wrapper is stripped (see `read_fsm_source`).
...
/// Extract just the FSM-relevant items from a (possibly Verus) source so syn
/// can parse them as plain Rust: the `TodoState` + `Event` enums and the named
/// transition function. Verus mode keywords / `ensures` / `requires` are
/// stripped. Proof functions and other non-Rust Verus constructs in the file
/// are never handed to syn.
fn read_fsm_source(
    path: &str,
    fn_name: &str,
    state_enum: &str,
