...
pub(crate) fn mutants(cfg: &Config, shard: Option<String>, baseline: Option<String>) -> Result<()> {
    ...
    // Inside the image, keep the `mutants.out` report out of the mounted project
    // tree (it would otherwise litter the user's working copy); send it to a
    // container-local path. The build copies already go to $TMPDIR.
