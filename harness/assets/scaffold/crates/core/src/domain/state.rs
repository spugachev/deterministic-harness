...
/// and `None` for an illegal one — total and panic-free, so Kani can
...
mod tests {
    ...
    fn legal_transitions() {
        ...
        // Done → Archived is its own arm (not folded into Active → Archived);
        // assert it so deleting that arm is a caught mutant, not a silent pass.
        assert_eq!(
            next(TodoState::Done, Event::Archive),
            Some(TodoState::Archived)
        );
    }
