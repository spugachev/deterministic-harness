...
- **`dhx verify --full` — after a big change / before release.** Adds only the
  slow instruments: Miri (~15 min), TSAN, cargo-mutants, fuzz, Loom, and the
  multi-seed DST sweep. Run it when you finish a feature, not on every save.

Every tier runs **inside the `dhx:latest` image** — there is no host `dhx`. Run
each command as `docker run --rm -v "$PWD":/work -w /work dhx:latest dhx <cmd>`
(an alias `dhx() { docker run --rm -v "$PWD":/work -w /work dhx:latest dhx "$@"; }`
makes it read like a local command). Use the `/check` and `/verify` skills.
